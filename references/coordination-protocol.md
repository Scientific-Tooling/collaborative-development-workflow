...
`3600s = 1800s` for the original attempt + `60s` for the bounded cancellation/status
...
1500s` effective minimum for one replacement. For broad, cross-cutting, high-risk,
long-running, or `review_set` work, use the extended tier:
`5400s = 3600s` initial + the same `60s + 120s + 120s + 1500s` reserves. The selected
