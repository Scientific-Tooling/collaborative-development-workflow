...
When calling `multi_agent_v1__spawn_agent`, use the `MODEL` and `REASONING_EFFORT`
resolved in the accepted `TaskSpec`, and pass both explicitly for every delegated
child. All child roles use `model: "gpt-5.6-luna"`; use
`reasoning_effort: "xhigh"` for simple, bounded, low-risk, or read-heavy work and
`reasoning_effort: "max"` for ambiguous, cross-cutting, high-risk, or long-running
work. Reviewer effort follows the review tier: standard integrated review uses
`xhigh`, while extended/high-risk/long-running/`review_set` review uses `max`.
Do not silently omit the override or accept runtime inheritance for this policy; if
the runtime applies a different profile, surface the mismatch as blocked or requiring
a user decision. Do not set `service_tier` unless the user requests it.
...
Before delegating a final review, finish the implementation and expected validation for that snapshot. Perform a pre-freeze self-check of likely edge cases and acceptance criteria so the reviewer is not started while the main agent still expects to make ordinary fixes. For a small, tightly coupled task, use one integrated final review unless a checkpoint carries a distinct risk that justifies an earlier review. Select the narrowest review tier, pin the reviewer `MODEL` and `REASONING_EFFORT` in its `TaskSpec`, and use the model policy in `SKILL.md`; do not inherit the parent settings implicitly. Invoke the reviewer in the foreground and wait for its terminal result; do not poll a moving review or inspect the frozen input repeatedly while it runs.
...
5. Ask the integrated reviewer, or every lane in the fixed review set, to review the refreshed snapshot, explicitly identifying resolved findings and unchanged scope. Keep the previously selected review tier/model/effort unless a fresh impact analysis justifies a documented change. Stop only when the integrated reviewer returns `STATUS: CLEAN`, or every required lane returns lane-scoped `STATUS: CLEAN` and the parent commits the aggregate coverage proof, with focused checks passing for that snapshot.
